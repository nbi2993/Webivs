...
// ================= MAIN COMPONENT LOADER OPTIMIZED =================
async function loadAndInject(url, placeholderId) {
    ...
    try {
        const response = await fetch(url, { cache: 'reload' }); // always reload for latest
        ...
        // Execute scripts sequentially (inline first, then external)
...
async function loadCommonComponents() {
    ...
    for (const comp of components) {
        if (document.getElementById(comp.id)) {
            ...
            if (success) {
                // Tự động gọi init nếu controller có, không cần kiểm tra từng loại
                const ctrlName = comp.id.replace('-placeholder', '').replace(/-([a-z])/g, (g) => g[1].toUpperCase()) + 'Controller';
                const ctrl = window[ctrlName];
    ...
    if (window.langSystem && typeof window.langSystem.initializeLanguageSystem === 'function') {
        window.langSystem.initializeLanguageSystem();
    } else {
